...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($resident->update($id, $household_number, $fname, $mname, $lname, $suffix, $gender, $dob, $civil_status, $nationality, $religion, $purok, $address, $mobile, $email, $voter_status, $precinct_number, $philhealth_number, $sss_gsis_number, $tin_number, $educational_attainment, $employment_status, $occupation, $monthly_annual_income, $pwd_status, $solo_parent_status, $relationship_household_head, $head_of_the_family, $type_of_dwelling, $health_condition, $vaccination_status, $blood_type, $beneficiary_program, $emergency_contact_person, $emergency_contact_relationship, $emergency_contact_number)) {
        echo json_encode(['success' => true, 'message' => 'Resident updated successfully']);
         ...
         if (isset($_SESSION['id']) && isset($_SESSION['role'])) {
            $descriptionLog = 'Resident ID: '. $id . ' - ' . $fname . ' ' . $mname . ' ' . $lname . ' ' . $suffix . 'resident updated succesfully';
            $userActivityLog->create($_SESSION['id'], $_SESSION['role'], 'Update', $descriptionLog);
        } else {
        ...
        // Validate session variables before logging activity
        if (isset($_SESSION['id']) && isset($_SESSION['role'])) {
            $descriptionLog = 'Resident ID: '. $id . ' - ' . $fname . ' ' . $mname . ' ' . $lname . ' ' . $suffix . 'failed to updateresident updated';
            $userActivityLog->create($_SESSION['id'], $_SESSION['role'], 'Update', $descriptionLog);
        } else {
            // Handle missing session variables
            error_log('Activity log error: Missing session variables for user ID or role.');
