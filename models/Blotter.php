...
class Blotter {
    ...
    // Method to check if a blotter exists
    public function exists($complainant_fname, $complainant_mname, $complainant_lname, $complainant_suffix, $date_created) {
        $query = "SELECT COUNT(*) FROM tbl_blotter WHERE complainant_fname = :complainant_fname AND complainant_mname = :complainant_mname AND complainant_lname = :complainant_lname AND complainant_suffix = :complainant_suffix AND date_created = :date_created";
    ...
    public function create($complainant_fname, $complainant_mname, $complainant_lname, $complainant_suffix, $complainant_address, $complainant_contact, $respondent_fname, $respondent_mname, $respondent_lname, $respondent_suffix, $respondent_address, $respondent_contact, $blotter_type, $details, $status) {
        
        // Check if blotter already exists
        if ($this->exists($complainant_fname, $complainant_mname, $complainant_lname, $complainant_suffix, $date_created)) {
            return ['success' => false, 'message' => 'Blotter already exists.'];
        }
        ...
        $stmt->bindParam(':complainant_fname', $complainant_fname);
        $stmt->bindParam(':complainant_mname', $complainant_mname);
        $stmt->bindParam(':complainant_lname', $complainant_name);
        $stmt->bindParam(':complainant_suffix', $complainant_suffix);
    ...
    public function update($id, $complainant_fname, $complainant_mname, $complainant_lname, $complainant_suffix, $complainant_address, $complainant_contact, $respondent_fname, $respondent_mname, $respondent_lname, $respondent_suffix, $respondent_address, $respondent_contact, $blotter_type, $details, $status) {
